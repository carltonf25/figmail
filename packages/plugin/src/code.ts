...
figma.ui.onmessage = async (msg) => {
  try {
    if (msg.type === "CONNECT") {
      ...
      try {
        // Use Figma's openExternal API to open the URL in browser
        figma.openExternal(oauthUrl);
        ...
        // Fallback: show manual instructions if openExternal fails
        const message = `
...
2. Grant permission for FigMail to access your templates and campaigns
3. You'll be redirected back and can continue using the plugin
        `.trim();
